                ...
                <div className="flex items-center gap-3">
                    {isMobile && (
                        <div className="relative">
                            ...
                            {mobileMenuOpen && (
                                <div className="absolute left-0 top-full mt-2 w-48 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg z-50 py-1">
                                    {user?.is_admin && (
                                        <>
                                            <button onClick={() => { navigate('/screener'); setMobileMenuOpen(false) }}
                                                className="w-full flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-800">
                                                <SlidersHorizontal className="w-4 h-4" />选股神器
                                            </button>
                                            <button onClick={() => { navigate('/briefing'); setMobileMenuOpen(false) }}
                                                className="w-full flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-slate-50 dark:hover:bg-slate-800">
                                                <Newspaper className="w-4 h-4" />盘前速递
                                            </button>
                                        </>
                                    )}

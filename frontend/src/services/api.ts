...
class ApiService {
    ...
    async getCapitalFlow(symbol: string, days?: number, signal?: AbortSignal): Promise<CapitalFlowResponse> {
        const params = new URLSearchParams({ symbol })
        if (days) params.append('days', String(days))
        return this.request<CapitalFlowResponse>(`/v1/capital-flow?${params}`, { signal })
    }
    ...
    async screener(filter: ScreenerFilter): Promise<ScreenerResponse> {
        return this.request<ScreenerResponse>('/v1/market/screener', {
            method: 'POST',
    ...
    async getTdSequential(symbol: string, startDate?: string, endDate?: string, period?: string, signal?: AbortSignal): Promise<TdResponse> {
        const params = new URLSearchParams({ symbol })
        if (startDate) params.append('start_date', startDate)
        if (endDate) params.append('end_date', endDate)
        if (period) params.append('period', period)
        return this.request<TdResponse>(`/v1/market/td-sequential?${params}`, { signal })
    }

    async getSupportResistance(symbol: string, startDate?: string, endDate?: string, period?: string, signal?: AbortSignal): Promise<SupportResistanceResponse> {
        const params = new URLSearchParams({ symbol })
        if (startDate) params.append('start_date', startDate)
        if (endDate) params.append('end_date', endDate)
        if (period) params.append('period', period)
        return this.request<SupportResistanceResponse>(`/v1/market/support-resistance?${params}`, { signal })
    }

...
def _extract_price_regex(text: Optional[str], price_type: str = "target") -> Optional[float]:
    ...
    if price_type == "target":
        patterns = [
            ...
            # ===== 新增：更宽松的匹配模式 =====
            # "建议目标价位"，包括"设定在XX元"形式 - 允许"位设"之间无空格
            r'建议目标[价位格](?:\s*[：:\|]|(?:设定在|设为))?\s*[¥$]?\s*(\d+(?:\.\d+)?)\s*元?',
            # "预计涨至XX元"
            r'预计[涨上行跌下降][^至到]{0,10}[至到]\s*[¥$]?\s*(\d+(?:\.\d+)?)\s*元?',
            # "看涨目标XX元" - 修正：不使用贪婪的字符类
            r'看[涨跌]目标\s*[：:\|]?\s*[¥$]?\s*(\d+(?:\.\d+)?)\s*元',
            # "涨至XX元附近"
            r'[涨跌升降][至到]\s*[¥$]?\s*(\d+(?:\.\d+)?)\s*元?\s*附近',
            # "目标区间XX-YY元"（取上限）
            r'目标区间\s*[：:\|]?\s*\d+(?:\.\d+)?\s*[-–—~至]\s*[¥$]?\s*(\d+(?:\.\d+)?)\s*元?',
            # "建议入场价格XX-YY元"（取上限）- 允许"格"后直接是数字
            r'建议入场[价格位](?:\s*[：:\|])?\s*(\d+(?:\.\d+)?)[-–—~至](\d+(?:\.\d+)?)\s*元',
            # "目标价格"
            r'目标价[格位]?\s*[：:\|]?\s*[¥$]?\s*(\d+(?:\.\d+)?)\s*元',
            # "盈利目标"
            r'盈利目标\s*[：:\|]?\s*[¥$]?\s*(\d+(?:\.\d+)?)\s*元',
        ]
    else:
        patterns = [
    ...
    for p in patterns:
        ...
        if m:
            try:
                # 处理区间情况（如"9.5-11元"）：如果有2个捕获组，取第2个（上限）
